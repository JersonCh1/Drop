...
// POST /api/suppliers/import-product - Importar producto desde URL (ADMIN)
router.post('/import-product', verifyAdmin, async (req, res) => {
  try {
    const { url, supplierId, categoryId, profitMargin } = req.body;

    if (!url) {
      return res.status(400).json({
        success: false,
        message: 'URL del producto es requerida'
      });
    }

    // Importar servicio de importación
    const { parseProductUrl, createProductFromImport, validateImportedProduct } = require('../services/productImporter');

    // Parsear URL del producto
    console.log(`📥 Importando producto desde: ${url}`);
    ...
    // Retornar producto para revisión antes de guardar
    res.json({
      success: true,
      message: 'Producto parseado exitosamente',
      data: {
        product,
        needsReview: productData.needsManualReview,
...
// POST /api/suppliers/sync-prices - Sincronizar precios con proveedores (ADMIN)
router.post('/sync-prices', verifyAdmin, async (req, res) => {
  try {
    const { supplierId } = req.body;

    // Obtener productos del proveedor
    const products = await prisma.product.findMany({
      where: supplierId ? { supplierId } : { supplierId: { not: null } },
      select: {
        id: true,
        name: true,
        supplierUrl: true,
    ...
    console.log(`🔄 Sincronizando precios de ${products.length} productos...`);

    const { syncProductPrice } = require('../services/productImporter');
    const results = [];

    // Sincronizar cada producto (en producción, hacer esto en background job)
    for (const product of products.slice(0, 10)) { // Limitar a 10 por request
      if (product.supplierUrl) {
        const syncResult = await syncProductPrice(product.id, product.supplierUrl);
        results.push({
