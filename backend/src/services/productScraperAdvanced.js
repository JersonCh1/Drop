...
/**
 * ALIEXPRESS - Scraping con HTTP (sin navegador)
 * Nota: AliExpress bloquea bots, así que el scraper puede fallar.
 * En ese caso, el usuario debe editar los datos manualmente.
 */
async function scrapeAliExpressProduct(url) {
  try {
    console.log(`🔍 Scraping AliExpress: ${url}`);
    console.log(`⚠️ NOTA: AliExpress bloquea scraping. Si falla, edita manualmente.`);
